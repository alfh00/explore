...
from constants.cex import CCXT_EXCHANGES
from constants.intervals import INTERVALS
...
class ExchangeDataManager:
    ...
    def __init__(self, exchange_name, path_download=DB_PATH) -> None:
        ...
        for path, subdirs, files in os.walk(self.path_download):
            for name in files:
                if os.path.join(path, name).endswith('.csv'):
                    ...
                    dir_path, filename = os.path.split(current_file)
                    filename, tail = os.path.split(dir_path)
                    try:
                        df_file = pd.read_csv(current_file)
                    except Exception:
                        continue
                    print( type(filename) )
                    # [_,_,_,_,_, exchange, timeframe] = str(dirname).split("//")
                    # print (exchange, timeframe)
                    # files_data.append({
